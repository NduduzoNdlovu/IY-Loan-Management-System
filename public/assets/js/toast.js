...
(function (window, document) {
  ...
  function injectStyles() {
    ...
    var css = ''
      ...
      + '.toast-card[data-confirm="1"]{width:360px;}'
      + '.toast-actions{display:flex;gap:8px;margin-top:10px;}'
      + '.toast-btn{flex:1 1 auto;font-size:12.5px;font-weight:600;padding:7px 10px;border-radius:8px;cursor:pointer;border:1px solid var(--toast-border);transition:background .15s ease,border-color .15s ease,opacity .15s ease;font-family:inherit;}'
      + '.toast-btn-secondary{background:transparent;color:var(--toast-text-muted);}'
      + '.toast-btn-secondary:hover{background:rgba(255,255,255,0.06);color:var(--toast-text);}'
      + '.toast-btn-primary{color:#0B0D11;}'
      + '.toast-btn-primary:hover{opacity:.9;}'
      + '.toast-card[data-type="success"] .toast-btn-primary{background:var(--toast-success);border-color:var(--toast-success);}'
      + '.toast-card[data-type="error"] .toast-btn-primary{background:var(--toast-error);border-color:var(--toast-error);}'
      + '.toast-card[data-type="warning"] .toast-btn-primary{background:var(--toast-warning);border-color:var(--toast-warning);}'
      + '.toast-card[data-type="info"] .toast-btn-primary{background:var(--toast-info);border-color:var(--toast-info);}'
  ...
  function confirm(message, options) {
    ...
    var title = options.title !== undefined ? options.title : (TITLES[type] || 'Confirm');
    ...
    return new Promise(function (resolve) {
      ...
      card.setAttribute('data-type', type);
      card.setAttribute('data-state', 'enter');
      card.setAttribute('data-confirm', '1');
      card.setAttribute('role', 'alertdialog');

      card.innerHTML = '<div class="toast-accent"></div>'
        + '<div class="toast-icon">' + (ICONS[type] || ICONS.warning) + '</div>'
        + '<div class="toast-body">'
        + (title ? '<p class="toast-title">' + escapeHtml(title) + '</p>' : '')
        + '<p class="toast-msg">' + escapeHtml(message) + '</p>'
        + '<div class="toast-actions">'
        + '<button type="button" class="toast-btn toast-btn-secondary" data-action="cancel">' + escapeHtml(cancelLabel) + '</button>'
        + '<button type="button" class="toast-btn toast-btn-primary" data-action="confirm">' + escapeHtml(confirmLabel) + '</button>'
        + '</div>'
        + '</div>';
      ...
      card.querySelector('[data-action="confirm"]').addEventListener('click', function () { settle(true); });
      card.querySelector('[data-action="cancel"]').addEventListener('click', function () { settle(false); });
    });
  }
